...
export default function CreateSectionDrawer({ isOpen, onClose, onSubmit, sources = [], chatbotId, user, initialData = null }) {
    ...
    // Reset or populate form when drawer opens/closes or initialData changes
    React.useEffect(() => {
        if (isOpen) {
            if (initialData) {
                setFormData({
                    ...
                    selectedSources: initialData.sources || [],
                    tone: initialData.tone || 'Neutral',
                    allowedTopics: initialData.scope?.allowed?.join(', ') || '',
                    blockedTopics: initialData.scope?.blocked?.join(', ') || ''
                });
            } else {
                setFormData({
                    name: '',
        ...
        <div className="fixed inset-0 z-50 overflow-hidden">
            ...
            <div className="absolute inset-y-0 right-0 w-full max-w-xl bg-[#0a0a0a] border-l border-white/10 shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out">
                ...
                <div className="flex items-center justify-between p-6 border-b border-white/5">
                    <div>
                        <h2 className="text-xl font-bold text-white">{initialData ? 'Edit Section' : 'Create Section'}</h2>
                        <p className="text-sm text-gray-500">Configure how the AI behaves for this specific topic.</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
                    <form id="create-section-form" onSubmit={handleSubmit} className="space-y-8">
                        {/* BASICS */}
                        <div>

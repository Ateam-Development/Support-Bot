...
export const REALTIME_PATHS = {
    ...
    chatbotStats: (chatbotId) => `chatbots/${chatbotId}/stats`,
};
...
/**
 * Subscribe to chatbot stats (e.g. global unread count)
 */
export function subscribeToChatbotStats(chatbotId, callback) {
    const db = getRealtimeDB();
    const statsRef = ref(db, REALTIME_PATHS.chatbotStats(chatbotId));

    onValue(statsRef, (snapshot) => {
        callback(snapshot.val() || { unreadCount: 0 });
    });

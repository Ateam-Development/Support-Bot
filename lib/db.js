...
async function saveFlow(chatbotId, flowData) {
    console.log(`DB: Saving flow for ${chatbotId}. NodeCount: ${Object.keys(flowData.nodes || {}).length}`);
    const firestore = getFirestore();
    ...
    console.log(`DB: Flow saved successfully for ${chatbotId}`);
...
async function getFlow(chatbotId) {
    console.log(`DB: Fetching flow for ${chatbotId}`);
    const firestore = getFirestore();
    const doc = await firestore.collection('flows').doc(chatbotId).get();

    if (!doc.exists) {
        console.warn(`DB: Flow document not found for ${chatbotId}`);
        return null;
    }

    const data = doc.data();
    console.log(`DB: Flow found for ${chatbotId}. StartNodeId: ${data.startNodeId}`);
...
async function updateConversation(conversationId, updates) {
    console.log(`DB: Updating conversation ${conversationId}`, JSON.stringify(updates));
    const firestore = getFirestore();
    const conversationRef = firestore.collection('conversations').doc(conversationId);

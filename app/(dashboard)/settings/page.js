...
export default function SettingsPage() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const [wsRes, cbRes] = await Promise.all([
                fetch('/api/workspace'),
                fetch('/api/chatbots')
            ]);
    ...
    const addOrigin = async (chatbotId, currentOrigins) => {
        ...
        try {
            await updateChatbotOrigins(chatbotId, newOrigins);
    ...
    const updateChatbotOrigins = async (chatbotId, allowedOrigins) => {
        // Optimistic update
        setChatbots(prev => prev.map(bot =>
            bot.id === chatbotId ? { ...bot, allowedOrigins } : bot
        ));
    ...
    if (loading) {
        return <div className="p-8 text-gray-400 font-mono animate-pulse">Loading workspace settings...</div>;
    }
                        ...
                        <div className="flex gap-3">
                            {editingWorkspaceName ? (
                                ...
                                <div className="flex-1 flex justify-between items-center bg-black/20 border border-white/10 rounded-xl px-4 py-3 group hover:border-white/20 transition-all">
                                    <span className="text-white font-medium text-lg">{workspace.name}</span>
                                    <button
